...
export class WorkerService {
  ...
  async getWorkers(hospitalUserId: bigint) {
    const hospital = await this.getHospitalByUserId(hospitalUserId);
    const workers = await this.prisma.worker.findMany({ where: { hospitalId: hospital.id }, include: { assignments: { where: { hospitalId: hospital.id, active: true }, include: { doctor: true }, take: 1 } }, orderBy: { createdAt: 'desc' } });
  ...
  async getWorker(hospitalUserId: bigint, workerId: bigint) {
    ...
    const assignment = await this.prisma.doctorWorkerAssignment.findFirst({ where: { workerId, active: true, hospitalId: worker.hospitalId! }, include: { doctor: true } });
  ...
  async addWorkerToHospital(hospitalUserId: bigint, workerId: bigint) {
    ...
    if (worker.hospitalId === hospital.id) return { workerId: Number(worker.id), workerCode: worker.workerCode, hospitalId: Number(hospital.id), hospitalRelationshipStatus: 'ACTIVE', message: 'Worker is already associated with this hospital' };
    if (worker.hospitalId !== null) throw new ConflictException('Worker is currently associated with another hospital');
    const updated = await this.prisma.worker.update({ where: { id: worker.id }, data: { hospitalId: hospital.id } });
    return { workerId: Number(updated.id), workerCode: updated.workerCode, hospitalId: Number(hospital.id), hospitalRelationshipStatus: 'ACTIVE', message: 'Worker added to hospital successfully' };
  }
  ...
  async terminateHospitalRelationship(hospitalUserId: bigint, workerId: bigint) {
    ...
    const activeEncounter = await this.prisma.encounter.findFirst({ where: { workerId: worker.id, hospitalId: hospital.id, status: 'ACTIVE' } });
    if (activeEncounter) throw new ConflictException('Cannot terminate the hospital relationship while the worker has an active visit. Complete the visit first.');
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.doctorWorkerAssignment.updateMany({ where: { workerId: worker.id, hospitalId: hospital.id, active: true }, data: { active: false, endedAt: new Date() } });
      return tx.worker.update({ where: { id: worker.id }, data: { hospitalId: null } });
    });
    return { workerId: Number(updated.id), workerCode: updated.workerCode, hospitalId: null, hospitalRelationshipStatus: 'TERMINATED', terminatedAt: new Date() };
  }
  ...
  async getMyProfile(userId: bigint) {
    const worker = await this.prisma.worker.findUnique({ where: { userId }, include: { hospital: true, assignments: { where: { active: true }, include: { doctor: true, hospital: true }, take: 1 } } });
    if (!worker) throw new NotFoundException('Worker profile not found');
    return { ...this.toResponse(worker), hospital: worker.hospital ? { id: Number(worker.hospital.id), code: worker.hospital.hospitalCode, name: worker.hospital.name, city: worker.hospital.city, district: worker.hospital.district } : null, assignedDoctor: worker.assignments[0] ? { id: Number(worker.assignments[0].doctor.id), name: worker.assignments[0].doctor.fullName, specialization: worker.assignments[0].doctor.specialization, assignedAt: worker.assignments[0].assignedAt } : null };
  }

  async getMyCurrentVisit(userId: bigint) {
    const worker = await this.prisma.worker.findUnique({ where: { userId } });
    if (!worker) throw new NotFoundException('Worker profile not found');
    const encounter = await this.prisma.encounter.findFirst({ where: { workerId: worker.id, status: 'ACTIVE' }, include: { hospital: true, doctor: true }, orderBy: { startedAt: 'desc' } });
    if (!encounter) return null;
    return { id: Number(encounter.id), hospitalId: Number(encounter.hospitalId), hospitalName: encounter.hospital.name, doctorId: Number(encounter.doctorId), doctorName: encounter.doctor.fullName, doctorSpecialization: encounter.doctor.specialization, startedAt: encounter.startedAt, status: encounter.status };
  }
  ...
  async getMyMedicalRecords(userId: bigint) {
    ...
    return records.map((record) => ({ id: Number(record.id), visitDate: record.visitDate, hospitalName: record.hospital.name, doctorName: record.doctor.fullName, symptoms: record.symptoms, diagnosis: record.diagnosis, treatment: record.treatment, notes: record.notes, updatedAt: record.updatedAt, prescriptions: record.prescriptions.map((p) => ({ id: Number(p.id), medicineName: p.medicineName, dosage: p.dosage, frequency: p.frequency, duration: p.duration, instructions: p.instructions })), attachments: record.attachments.map((a) => ({ id: Number(a.id), fileName: a.fileName, mimeType: a.mimeType, fileSize: a.fileSize, status: a.status, createdAt: a.createdAt })) }));
  }
  ...
  private withAssignment(response: any, assignment: any) {
    ...
    return { ...response, assignedDoctor: { id: Number(assignment.doctor.id), name: assignment.doctor.fullName, specialization: assignment.doctor.specialization, assignedAt: assignment.assignedAt }, assignedDoctorId: Number(assignment.doctor.id), assignedDoctorName: assignment.doctor.fullName, assignedDoctorSpecialization: assignment.doctor.specialization };
  }
