...
const roleNavigation = {
  ...
  DOCTOR: [
    ...
    { to: "/doctor/workers", label: "Assigned Workers" },
  ],
  WORKER: [
...
const roleNames = { HOSPITAL: "Hospital staff", DOCTOR: "Doctor", WORKER: "Worker" };
    ...
    <div className="app-shell">
      ...
      <main className="content-area">
        <header className="page-header">
          <div><p className="eyebrow">{roleNames[user?.role] || "DHRMS"}</p><h1>{title}</h1>{description && <p className="page-description">{description}</p>}</div>
          {actions?.length > 0 && <div className="header-actions">{actions.map((action) => <button key={action.label} type="button" className={action.variant === "secondary" ? "button button-secondary" : "button button-primary"} onClick={action.onClick}>{action.label}</button>)}</div>}
        </header>
