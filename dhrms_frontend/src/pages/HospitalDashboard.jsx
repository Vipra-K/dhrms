...
const HospitalDashboard = () => {
  ...
  return <RoleLayout title={hospital.name} description={`${hospital.code}${hospital.city ? ` · ${hospital.city}` : ""}`} actions={[
    ...
    { label: "Find worker (Phone/ID)", onClick: () => navigate("/hospital/find-worker"), variant: "secondary" },
    { label: "Manage doctors", onClick: () => navigate("/hospital/doctors"), variant: "secondary" },
  ]}>
    <div className="dashboard-welcome"><div><span className="eyebrow">Today at a glance</span><h2>Manage care, not permanent worker assignments.</h2><p>Scan a worker's permanent DHRMS QR or search by Phone Number / Worker ID to start a visit with the appropriate doctor.</p></div><div className="dashboard-mark">H</div></div>
    <div className="card-row">
      ...
      <article className="card"><span className="eyebrow">Scan QR</span><h2>QR</h2><p>Identify workers through their permanent DHRMS QR card.</p><button className="button button-primary" onClick={() => navigate("/hospital/workers/scan")}>Scan QR</button></article>
      <article className="card"><span className="eyebrow">Search worker</span><h2>Search</h2><p>Look up worker by phone number or DHRMS worker ID.</p><button className="button button-secondary" onClick={() => navigate("/hospital/find-worker")}>Find worker</button></article>
    </div>
    <div className="panel"><div className="section-toolbar"><div><span className="eyebrow">Current healthcare relationships</span><h2>Active visits</h2></div><button className="button button-secondary" onClick={() => navigate("/hospital/workers")}>View all</button></div>{activeVisits.length === 0 ? <div className="empty-state-card"><h3>No active visits</h3><p>Scan a worker QR or search by phone/ID to begin the next consultation.</p></div> : <div className="table-card"><table className="table"><thead><tr><th>Worker</th><th>Doctor</th><th>Started</th><th>Status</th></tr></thead><tbody>{activeVisits.map((visit) => <tr key={visit.id}><td><div className="person-cell"><span className="avatar">{(visit.workerName || "W").charAt(0)}</span><div><strong>{visit.workerName}</strong><small>{visit.workerCode}</small></div></div></td><td>{visit.doctorName}</td><td>{new Date(visit.startedAt).toLocaleString()}</td><td><span className="status-badge status-active">ACTIVE</span></td></tr>)}</tbody></table></div>}</div>
  </RoleLayout>;
};
