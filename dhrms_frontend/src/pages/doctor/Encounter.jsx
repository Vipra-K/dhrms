  ...
  return <RoleLayout title="Visit details" description={encounter ? `${encounter.workerName} · ${encounter.hospitalName}` : "Current visit"} actions={[{ label: "Refresh", onClick: load, variant: "secondary", disabled: loading }, { label: "Back", onClick: () => navigate("/doctor/workers"), variant: "secondary" }]}>
    ...
    {encounter && <>
      ...
      {encounter.status === "ACTIVE" ? <>
        <div className="panel"><div className="section-toolbar"><div><h2>Health history</h2><p>Use the summary as a quick reference. Check the original records before making clinical decisions.</p></div><button className="button button-primary" type="button" onClick={analyzeHistory} disabled={aiLoading}>{aiLoading ? "Loading…" : "Summarize history"}</button></div>{aiError && <div className="alert error">{aiError}</div>}{aiSummary && <div className="ai-summary" style={{ marginTop: 16, whiteSpace: "pre-wrap", lineHeight: 1.6 }}><strong>History summary</strong><p>{aiSummary.summary}</p><small>{aiSummary.recordsAnalyzed} record{aiSummary.recordsAnalyzed === 1 ? "" : "s"} reviewed · Verify against original records.</small></div>}</div>
