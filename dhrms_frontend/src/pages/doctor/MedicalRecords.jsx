...
const MedicalRecords = ({ workerId }) => {
  ...
  const loadRecords = async () => {
    try { setError(""); setRecords(await getMedicalRecords(workerId)); }
    catch (err) { setError(getApiError(err, "Unable to load medical records.")); }
    finally { setLoading(false); }
  };
  useEffect(() => { loadRecords(); }, [workerId]);
  ...
  const submitRecord = async (e) => {
    ...
    try {
      ...
      setRecordForm(blankRecord); setRecordTarget(null); setShowRecordForm(false);
    } catch (err) { setError(getApiError(err, "Unable to save medical record.")); }
  ...
  const submitPrescription = async (e) => {
    ...
    try {
      ...
      setRecords((current) => current.map((record) => {
        if (record.id !== (prescriptionTarget?.medicalRecordId || prescriptionRecordId)) return record;
      ...
      setPrescriptionForm(blankPrescription); setPrescriptionTarget(null); setPrescriptionRecordId(null);
    } catch (err) { setError(getApiError(err, "Unable to save prescription.")); }
    finally { setSaving(false); }
  };

  if (loading) return <div className="loading-card">Loading medical history…</div>;
  return <section className="medical-records-section">
    <div className="section-toolbar"><div><span className="eyebrow">Clinical history</span><h2>Medical records</h2><p>{records.length} recorded visit{records.length === 1 ? "" : "s"}. Records are preserved rather than hard-deleted.</p></div><button className="button button-primary" onClick={() => { setRecordTarget(null); setRecordForm(blankRecord); setShowRecordForm(true); }}>+ Add visit</button></div>
    {error && <div className="alert error">{error}</div>}
    {records.length === 0 ? <div className="empty-state-card"><h3>No medical records yet</h3><p>Add the first clinical visit for this worker.</p></div> : <div className="record-timeline">{records.map((record) => <article className="medical-record-card" key={record.id}><div className="record-date"><strong>{record.visitDate || "—"}</strong><span>Visit</span></div><div className="record-body"><div className="record-heading"><div><span className="status-badge status-active">Diagnosis</span><h3>{record.diagnosis}</h3></div><div className="row-actions"><button className="button button-ghost button-small" onClick={() => { setRecordTarget(record); setRecordForm({ visitDate: record.visitDate || "", symptoms: record.symptoms || "", diagnosis: record.diagnosis || "", treatment: record.treatment || "", notes: record.notes || "" }); setShowRecordForm(true); }}>Edit</button></div></div><div className="record-fields"><div><small>Symptoms</small><p>{record.symptoms || "—"}</p></div><div><small>Treatment</small><p>{record.treatment || "—"}</p></div><div><small>Notes</small><p>{record.notes || "—"}</p></div></div><div className="prescription-section"><div className="prescription-header"><h4>Prescriptions</h4><button className="button button-secondary button-small" onClick={() => { setPrescriptionTarget(null); setPrescriptionForm(blankPrescription); setPrescriptionRecordId(record.id); }}>+ Add prescription</button></div>{record.prescriptions?.length ? <div className="prescription-list">{record.prescriptions.map((item) => <div className="prescription-item" key={item.id}><div><strong>{item.medicineName}</strong><span>{[item.dosage, item.frequency, item.duration].filter(Boolean).join(" · ") || "No dosage details"}</span>{item.instructions && <small>{item.instructions}</small>}</div><button className="button button-ghost button-small" onClick={() => { setPrescriptionTarget(item); setPrescriptionForm({ medicineName: item.medicineName || "", dosage: item.dosage || "", frequency: item.frequency || "", duration: item.duration || "", instructions: item.instructions || "" }); }}>Edit</button></div>)}</div> : <p className="muted-note">No prescriptions for this visit.</p>}</div></div></article>)}</div>}
