...
const MedicalRecords = ({ workerId }) => {
  ...
  const submitRecord = async (e) => {
    ...
    try {
      ...
      if (attachmentFiles.length) attachments = [...attachments, ...(await uploadMedicalAttachments(saved.id, attachmentFiles))];
      const withAttachments = { ...saved, attachments };
      setRecords((current) => recordTarget ? current.map((r) => r.id === saved.id ? withAttachments : r) : [withAttachments, ...current]);
  ...
  const revokeAttachment = async (recordId, attachmentId) => {
    if (!window.confirm("Revoke this attachment? It will no longer be available to the worker.")) return;
    try {
      await revokeMedicalAttachment(attachmentId);
      setRecords((current) => current.map((record) => record.id === recordId ? { ...record, attachments: record.attachments.filter((item) => item.id !== attachmentId) } : record));
