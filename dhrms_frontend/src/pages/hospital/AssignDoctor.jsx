...
const AssignDoctor = () => {
  ...
  const handleAssign = async (event) => {
    ...
    try {
      ...
      setWorkers((current) => current.map((worker) => String(worker.id) === String(selectedWorker.id)
        ? { ...worker, assignedDoctor: { ...selectedDoctor } }
        : worker));
  ...
  return (
    <RoleLayout
      title="Assign Doctor"
      description="Assign an active hospital doctor to a worker without starting a new visit."
      actions={[
        { label: "Manage Workers", onClick: () => navigate("/hospital/manage-workers"), variant: "secondary" },
      ...
      <div className="scanner-layout">
        <div className="panel">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">Worker selection</span>
              <h2>Choose a worker</h2>
              <p>Only workers currently associated with this hospital are listed.</p>
            </div>
            <button className="button button-secondary" type="button" onClick={loadData} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</button>
          </div>

          <div className="field">
            <label htmlFor="worker-search">Search workers</label>
            <input id="worker-search" className="input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, phone or worker code" />
          </div>

          {loading ? (
            <div className="loading-card">Loading hospital workers…</div>
          ) : filteredWorkers.length === 0 ? (
            <div className="empty-state-card">
              <h3>No matching workers</h3>
              <p>Try another search or add a worker to this hospital first.</p>
              <button className="button button-primary" type="button" onClick={() => navigate("/hospital/find-worker")}>Find a worker</button>
            </div>
          ) : (
            <div className="assignment-list" role="radiogroup" aria-label="Workers">
                ...
                return (
                  <button key={worker.id} className={`assignment-option ${selected ? "selected" : ""}`} type="button" role="radio" aria-checked={selected} onClick={() => { setWorkerId(String(worker.id)); setSuccess(""); setError(""); }}>
                    ...
                    <span className="assignment-option-copy">
                      ...
                      <small>{worker.phone || worker.workerCode || "Worker record"}</small>
                    </span>
                    <span className="assignment-option-doctor">{currentDoctor}</span>
                  </button>
                );
              })}
            </div>
        ...
        <aside className="card">
          ...
          <p>{selectedWorker ? "Choose the active doctor responsible for this worker." : "Select a worker from the list to continue."}</p>

          <form onSubmit={handleAssign}>
            <div className="field">
              <label htmlFor="assignment-doctor">Active doctor</label>
              <select id="assignment-doctor" className="select" value={doctorId} onChange={(event) => { setDoctorId(event.target.value); setSuccess(""); }} disabled={!selectedWorker || doctors.length === 0}>
                <option value="">Select a doctor</option>
                {doctors.map((doctor) => <option key={doctor.id} value={doctor.id}>{doctor.fullName}{doctor.specialization ? ` · ${doctor.specialization}` : ""}</option>)}
              </select>
            </div>
            ...
            <div className="modal-actions">
              ...
              <button className="button button-primary" type="submit" disabled={assigning || !selectedWorker || !selectedDoctor}>{assigning ? "Assigning…" : "Assign doctor"}</button>
            </div>
          </form>
        </aside>
      </div>
