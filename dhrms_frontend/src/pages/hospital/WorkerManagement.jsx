...
const WorkerManagement = () => {
  ...
  const loadWorkers = async () => {
    try { setError(""); setWorkers(await getWorkers()); }
    catch (err) { setError(getApiError(err, "Unable to load workers.")); }
  ...
  const showQr = async (action) => {
    try { setError(""); setSelectedQr(await action()); }
    catch (err) { setError(getApiError(err, "Unable to load QR code.")); }
  };
