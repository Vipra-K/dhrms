...
export const lookupWorkerByQr = async (qrContent) => {
  const response = await api.post("/hospitals/workers/qr/lookup", { qrContent });
  return response.data;
};
...
export const assignWorkerToDoctor = async (workerId, doctorId) => {
  const response = await api.post(`/hospitals/workers/${workerId}/assignment`, { doctorId });
  return response.data;
};
...
export const updateMyWorkerProfile = async (worker) => {
  const response = await api.put("/workers/me", worker);
  return response.data;
};

export const getMyWorkerMedicalRecords = async () => {
